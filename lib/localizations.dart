...
class Localized {
  ...
  static Map<String, Map<String, String>> _localizedValues = {
    'en': {
      ...
      'ex_temp_save_mb': 'External storage max size',
      'ex_temp_save_mb_desc':
          'If the temporary folder (Temp) exceeds its capacity, old files will be deleted. Non upload will not be uploaded automatically. If it fails, please upload it manually from the in-app data later.',
      'in_save_mb': 'In-App max size',
      'in_save_mb_desc':
          'Old data will be deleted when in-app data exceeds the max size.',
...
''',
    ...
    'ja': {
      ...
      'ex_temp_save_mb': '外部ストレージ一時容量',
      'ex_temp_save_mb_desc':
          '一時フォルダ(Temp)が容量を超えると古いものが削除されます。Non uploadは自動的にアップロードされません。失敗したときなどは後でアプリ内データから手動でアップロードしてください。',
      'in_save_mb': 'アプリ内データ最大容量',
      'in_save_mb_desc': 'アプリ内データが最大容量を超えると古いものが削除されます。',
      'timer_mode': '自動停止',
      'timer_mode_desc':
          '自動的に撮影を停止します。バッテリー残量10%以下で停止します。アプリがバックグラウンドになると停止します。',
